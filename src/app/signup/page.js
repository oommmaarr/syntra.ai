...
import { GraduationCap, Users, UserStar, Eye, EyeOff } from "lucide-react";
...
export default function SignupPage() {
  ...
  const handleValidation = () => {
    ...
    if (!form.fullname) {
      ...
      newErrors.fullname = "Minimum 4 characters";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    clearError();
    const result = await signup(form.fullname, form.email, form.password, form.role);
    if (result.success) {
      router.push("/roles");
    }
  };
      ...
      <div className="flex w-full min-h-screen flex-col items-center justify-center bg-white  lg:w-[50%] xl:px-12 lg:px-8 p-6">
        <div className="w-full h-full space-y-6 lg:space-y-8">
          ...
          <div className="xl:px-22 px-1 space-y-10">
            <div className="space-y-2 ">
              ...
              <p className="lg:text-base text-sm text-[#1B1B1B] mt-3">
                Join the ecosystem. Choose your role to get started.
              </p>
            </div>
            ...
            <div className="lg:px-12 space-y-10">
              ...
              <div className="grid grid-cols-3 gap-4 ">
                {[
                  ...
                  { id: "recruiter", icon: <UserStar />, label: "Recruiter" },
                ].map((r) => (
                  <button
                    key={r.id}
                    type="button"
                    onClick={() => handleRoleChange(r.id)}
                    className={`flex flex-col items-center justify-center rounded-2xl border py-4 transition-all duration-200 cursor-pointer ${
                      form.role === r.id
                        ? "border-[#0094BD] ring-1 ring-[#0094BD] bg-blue-50 text-[#0094BD]"
                ...
                <div className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <div className="relative mt-2">
                      <div className="absolute left-3 top-[15px] text-gray-900 font-bold">
                        <svg
                ...
                <div>
                  <div className="flex items-center space-x-3 pt-2">
                    <input
                      ...
                      className="h-5 w-5 rounded border-gray-300 text-[#0094BD] focus:ring-[#0094BD]"
                    />
                    <label
                      htmlFor="terms"
                      className="lg:text-sm text-[11px] leading-none font-medium text-black select-none"
                    >
                      I agree to the{" "}
                      <Link
                        href="/terms"
                        className="text-[#1387AE] font-bold hover:underline"
                      >
                        Terms of Service
                      </Link>{" "}
                      and{" "}
          ...
          <div className=" text-center text-xs text-gray-600 flex flex-col items-center justify-end ">
            <p className="border-t w-full  border-gray-400 mb-4"></p>
